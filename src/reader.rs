...
impl Reader {
    pub fn new(args: Args) -> Self {
        Reader { args, counter: 1, last_line_empty: false }
    }
    ...
    fn read<T: Read>(&mut self, inner: T, source: InputSource) {
        ...
        loop {
            ...
            if self.on_line(line, &mut buf).is_err() { return; }
        }
    }

    fn on_line(&mut self, data: Vec<u8>, buf: &mut BufWriter<StdoutLock>) -> Result<(), Error> {
        let tab = if self.args.show_tabs { &[b'^', b'I'][..] } else { &[b'\t'][..] };
        let new_line = if self.args.show_ends { &[b'$', b'\n'][..] } else { &[b'\n'][..] };
        let is_empty = data.len() <= 2 && (data[..] == [b'\r', b'\n'][..] || data[0] == b'\n' || data[0] == b'\r');

        if self.args.squeeze_blank && self.last_line_empty && is_empty { return Ok(()); }
        self.last_line_empty = is_empty;

        let mut buffer: Vec<u8> = Vec::with_capacity(if self.args.show_non_printing { 1024 *1024 } else { data.len() });
